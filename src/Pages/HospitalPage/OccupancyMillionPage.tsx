...
export default function OccupancyMillionPage() {
  const countryList: any = useCountriesDropdown();

  const [country, setCountry] = useState("");
  const url = `${process.env.REACT_APP_BASE_URL}/hospital/country/occupancy/million/${country}`;
    ...
    datasets: [
      {
        ...
        data: !data[0]
          ? null
          : data[0].country.map(
              (n: HospitalData) => n.Daily_ICU_occupancy_per_million
            ),
      },
      {
        ...
        data: !data[0]
          ? null
          : data[0].country.map(
              (n: HospitalData) => n.Daily_hospital_occupancy_per_million
            ),
      },
    ],
  };
